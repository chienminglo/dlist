...
static void locker_pthread_destroy(Locker* thiz)
{
	...
	ret = pthread_mutexattr_destroy(&(priv->attr));
...
Locker* locker_create(void)
{
	...
	if(thiz != NULL)
	{
		...
		pthread_mutexattr_init(&(priv->attr));
		pthread_mutexattr_settype(&(priv->attr), PTHREAD_MUTEX_RECURSIVE);
		pthread_mutex_init(&(priv->mutex), &(priv->attr));
	}
